...
@client_router.post(
    ...
    summary="Rota para criação de cliente",
    description="Somente admins podem criar um cliente."
)
def create_client(client: ClientCreateSchema, client_service: ClientService = Depends(get_client_service), admin_client: ClientModel = Depends(require_admin)):
    created_client = client_service.add_client(client.name, client.email, client.password, client.role)
    return ClientSchema(
...
@client_router.get(
    ...
    summary="Rota para listar todos os clientes",
    description="Rota pública e qualquer role de cliente pode acessar."
)
...
@client_router.get(
    ...
    status_code=HTTPStatus.OK,
    response_model=ClientSchema,
    summary="Rota para listar um cliente em específico",
    description="Rota pública e qualquer role de cliente pode acessar."
)
...
@client_router.delete(
    path='/delete',
    status_code=HTTPStatus.OK,
    response_model=DeleteClientSchema,
    summary="Rota para deletar um cliente",
    description="Somente clientes com role de admin, podem deletar outros clientes."
)
...
@client_router.patch(
    ...
    status_code=HTTPStatus.OK,
    response_model=ClientSchema,
    summary="Rota para atualizar um cliente",
    description="O cliente logado pode atualizar somente os seus próprios dados."
)
